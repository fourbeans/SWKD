...
def main():
    ...
    # 输出目录添加蒸馏相关参数
    out_dir = f'Resnet19_{args.method}_distill_{args.teacher}_T{args.T}_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_a{args.alpha}_b{args.feature_beta}_g{args.logit_gamma}_{args.suffix}'
    ...
    logger = get_logger(os.path.join(pt_dir,
                                     f'Resnet19_{args.method}_distill_{args.teacher}_T{args.T}_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_{args.suffix}' + '.log'))
    logger.info('start training with knowledge distillation!')


    if args.method == 'ann':
        student_net = resnet19_ann(num_classes=10)
    elif args.method == 'lif':
        student_net = resnet19_lif(num_classes=10, T=args.T)

    if args.teacher == 'ann':
        teacher_net = resnet19_ann(num_classes=10)
    elif args.teacher == 'lif':
        teacher_net = resnet19_lif(num_classes=10, T=args.T)
