...
class Cifar100_C(Dataset):
    """
    CIFAR-100-C dataset with the option to split into subsets.
    """
    def __init__(self, root=r'E:\dataset\CIFAR_100_C', distor='gaussian_noise', severity=0, subset_type='Full', transform=None):
        """
        Args:
            root (str): Path to the dataset folder.
            distor (str): The type of distortion (e.g., 'gaussian_noise').
            severity (int): Severity level (0-4).
            subset_type (str): Type of dataset subset ('Tiny', 'Mid', 'Full').
            transform (callable, optional): A function/transform to apply to the image.
        """
        super(Cifar100_C, self).__init__()

        # Load the data and labels using the original logic
        data_file_name = os.path.join(root, distor + '.npy')
        self.data = np.load(data_file_name)[severity*10000: (severity+1)*10000]
        
        labels_file_name = os.path.join(root, distor + '_labels.npy')
        self.labels = np.load(labels_file_name)[severity*10000: (severity+1)*10000]
        
        # Select the subset based on subset_type
        if subset_type == 'Tiny':
            # Subset with the first 10 classes
            self.data = self.data[np.isin(self.labels, range(10))]
            self.labels = self.labels[np.isin(self.labels, range(10))]
        elif subset_type == 'Mid':
            # Subset with the first 50 classes
            self.data = self.data[np.isin(self.labels, range(50))]
            self.labels = self.labels[np.isin(self.labels, range(50))]
        # 'Full' is the default and doesn't require additional filtering
