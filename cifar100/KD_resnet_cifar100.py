...
def main():
    dataset_root_dir = '/zjh/data/cifar100'

    parser = argparse.ArgumentParser(description='Knowledge Distillation for CIFAR100')
    ...
    transform_train = ClassificationPresetTrain(mean=(0.5071, 0.4867, 0.4408),
    ...
    # 输出目录添加蒸馏相关参数
    out_dir = f'cifar100_Resnet19_{args.method}_distill_{args.teacher}_T{args.T}_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_a{args.alpha}_b{args.feature_beta}_g{args.logit_gamma}_{args.suffix}'
    ...
    logger = get_logger(os.path.join(pt_dir,
                                     f'cifar100_Resnet19_{args.method}_distill_{args.teacher}_T{args.T}_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_{args.suffix}' + '.log'))
    logger.info('start training with knowledge distillation!')

    # 定义学生模型和教师模型
    if args.method == 'ann':
        student_net = resnet19_ann(num_classes=100)
    elif args.method == 'lif':
        student_net = resnet19_lif(num_classes=100, T=args.T)

    if args.teacher == 'ann':
        teacher_net = resnet19_ann(num_classes=100)
    elif args.teacher == 'lif':
        teacher_net = resnet19_lif(num_classes=100, T=args.T)
    ...
    if args.mode == 'test':
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            remaining_time = epoch_time * (args.epochs - epoch - 1) / 3600  # 小时
