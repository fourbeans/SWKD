...
class ResNet(nn.Module):
    ...
    #
    #     out = F.avg_pool2d(out, 8)
    #     out = out.view(out.size(0), -1) #等价于 flatten(x, 1) [B, C, H, W] → [B, C*H*W]
    #     feature.append(out)
    #
    #     out = self.fc(out)
    #     return out, feature
...
def resnet19_ann(num_classes=10,step = 3):
    return ResNet(ResidualBlock, step=step,num_classes=num_classes, dim=[3, 3, 2],channel=[64,128,256,512])


if __name__ == "__main__":
   
    model_tresnet19 = resnet19_ann(256)
    
    # 打印每个参数的名字和形状
    for name, param in model_tresnet19.state_dict().items():
