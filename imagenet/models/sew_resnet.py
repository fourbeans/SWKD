...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn1 = neuron.MultiStepLIFNode(surrogate_function=surrogate.ATan(), backend='cupy', decay_input=False)


        self.stride = stride
        self.sn2 = neuron.MultiStepLIFNode(surrogate_function=surrogate.ATan(), backend='cupy', decay_input=False)

        self.sn3 = neuron.MultiStepLIFNode(surrogate_function=surrogate.ATan(), backend='cupy', decay_input=False)

    def forward(self, x):
        identity = x

        out = self.sn1(divide(self.bn1(self.conv1(resotre(x)))))

        out = self.sn2(divide(self.bn2(self.conv2(resotre(out)))))



        if self.downsample is not None:
            identity = self.sn3(divide(self.downsample(resotre(identity))))
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn4 = neuron.MultiStepLIFNode(surrogate_function=surrogate.ATan(), backend='cupy', decay_input=False)
    ...
    def forward(self, x):
        ...
        out = self.sn3(divide(self.bn3(self.conv3(resotre(out)))))

        if self.downsample is not None:
            identity = self.sn4(divide(self.downsample(resotre(identity))))
...
class SEWResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.transform1 = Feature_Converter(64* block.expansion)
        self.transform2 = Feature_Converter(128* block.expansion)
        self.transform3 = Feature_Converter(256* block.expansion)
        self.transform4 = Feature_Converter(512* block.expansion)
    ...
    def _forward_impl(self, x):
        ...
        x = self.conv1(resotre(x))
        x = divide(self.bn1(x))
        x = self.sn1(x)
        x = divide(self.maxpool(resotre(x)))
        ...
        x = self.layer4(x)
        if self.training:
            feature.append(self.transform4(torch.mean(x, dim=0)))
